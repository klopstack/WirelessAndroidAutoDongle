...
- `AAWG_WIFI_MODE`:
  - `ap`: the dongle creates a wifi network for the phone to join
  - `sta`: the dongle joins an existing wifi network (SSID/password must be set)
- `AAWG_WIFI_SSID` / `AAWG_WIFI_PASSWORD`: used for both modes (advertised in AP mode, joined in STA mode)

Note: `/etc/wpa_supplicant.conf` and `/etc/network/interfaces` are autogenerated at boot from `aawgd.conf`.

## OTA updates
This image supports A/B rootfs OTA updates (SWUpdate), typically triggered via MQTT.

- **Key material**: the SWUpdate verification public key is stored in the image at `/etc/swupdate/public.pem` (sourced from `aa_wireless_dongle/board/common/rootfs_overlay/etc/swupdate/public.pem`). Replace the placeholder with your real public key; keep the private key in CI/build.
- **Docs**: see [BUILDING.md](BUILDING.md) for example `openssl` key generation commands and the MQTT `ota <url>` trigger format.
...
- Configure wifi mode/credentials as needed (see `aawgd.conf`) and enable SSH by setting `AAWG_ENABLE_SSH=1`. See [the instructions to update the configurations](#configurations).
...
- Connect to the device:
  - If using **AP mode** (`AAWG_WIFI_MODE=ap`): join the dongle's wifi SSID/password as configured in `aawgd.conf` (SSID may default to `AAWirelessDongle` if unset; password may be randomly generated if unset).
  - If using **STA mode** (`AAWG_WIFI_MODE=sta`): connect to the dongle using whatever IP it received from your existing wifi network (SSID/password must be explicitly configured).
